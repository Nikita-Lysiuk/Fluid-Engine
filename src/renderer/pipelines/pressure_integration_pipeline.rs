...
mod cs {
    ...
    shader! {
        ty: "compute",
        path: "shaders\\compute\\pressure_integration.comp"
    }
}

pub struct PressureIntegrationPipeline {
    pub pipeline: Arc<ComputePipeline>,
    descriptor_set: Option<Arc<DescriptorSet>>,
    dispatch_count: u32,
}

impl ComputeStep for PressureIntegrationPipeline {
    ...
    fn prepare(
        ...
        self.descriptor_set = Some(DescriptorSet::new(
            ...
            layout.clone(),
            [
                WriteDescriptorSet::buffer(0, physics_data.position_b.clone()),
                WriteDescriptorSet::buffer(1, physics_data.pressure_accelerations.clone()),
                WriteDescriptorSet::buffer(2, sim_params.clone()),
                WriteDescriptorSet::buffer(3, physics_data.velocity_a.clone()),
                WriteDescriptorSet::buffer(4, physics_data.position_a.clone()),
            ],
            []
        ).unwrap());
    }
    fn execute<Cb>(&self, builder: &mut AutoCommandBufferBuilder<Cb>) {
        let set = self.descriptor_set.as_ref().expect("PressureIntegrationPipeline: call prepare() before execute()");
        builder
