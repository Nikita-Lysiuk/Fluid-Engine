...
mod cs {
    ...
    shader!(ty: "compute", path: "shaders\\compute\\viscosity.comp");
}
...
impl ComputeStep for ViscosityPipeline {
    ...
    fn execute<Cb>(
        ...
        let set = DescriptorSet::new(
            ...
            layout.clone(),
            [
                WriteDescriptorSet::buffer(0, physics_data.velocity_b.clone()),
                WriteDescriptorSet::buffer(1, physics_data.position_b.clone()),
                WriteDescriptorSet::buffer(2, sim_params.clone()),
                WriteDescriptorSet::buffer(3, physics_data.grid_entries.clone()),
                WriteDescriptorSet::buffer(4, physics_data.grid_start.clone()),
