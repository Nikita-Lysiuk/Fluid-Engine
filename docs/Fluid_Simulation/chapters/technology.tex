Przedstawione w~poprzednim rozdziale podstawy teoretyczne --- od eulerowskiego
i~lagrange'owskiego opisu ruchu, przez sformułowanie metody SPH, aż po techniki bezpośredniego
...
siatki eulerowskiej z~elastycznością cząstek lagrange'owskich \cite{brackbill1986flip}. 
...
eulerowska, a~cząstki lagrange'owskie pełnią rolę tracerów adwekcji ---
...
renderowany niezależnie za pomocą rendererów Cycles lub EEVEE. Podejście to sprawdza się doskonale
w~produkcji filmowej i~reklamowej, lecz ze swej natury wyklucza interaktywność i~sprzężenie zwrotne w~czasie 
...
\begin{figure}[ht]
    ...
    \caption{Interfejs symulacji FLIP w~programie Blender z~widocznym panelem konfiguracji domeny i~podglądem cieczy. Źródło: Własne opracowanie na podstawie dokumentacji \cite{blenderdocs2023}}
    \label{fig:blender_flip}
\end{figure}

\subsection{Symulacja FLIP w~silniku Unreal Engine 5}

Unreal Engine 5 jest zaawansowanym silnikiem aplikacji interaktywnych, rozwijanym przez Epic Games
i~stosowanym zarówno w~produkcji gier, jak i~przy tworzeniu efektów wizualnych dla filmów i~symulacji \cite{ue5fluids}. 
...
\begin{figure}[ht]
    ...
    \caption{Interfejs edytora Niagara w~silniku Unreal Engine 5 z~aktywną symulacją 3D FLIP zrealizowaną przy użyciu wtyczki NiagaraFluids. Źródło: Własne opracowanie na podstawie dokumentacji \cite{ue5fluids}}
    \label{fig:ue5_flip}
\end{figure}
...
W~odróżnieniu od dwóch poprzednich rozwiązań, biblioteka ta opiera się wyłącznie na metodzie SPH
w~formie cząsteczkowej, bez stosowania pomocniczej siatki eulerowskiej. Pod względem przyjętego modelu 
...
SPlisHSPlasH jest szeroko stosowana w~środowisku naukowym jako platforma referencyjna do testowania
i~porównywania nowych algorytmów SPH \cite{koschier2020smoothed}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/splishsplash.png}
    \caption{Symulacja cieczy w~bibliotece SPlisHSPlasH z~aktywnym solverem DFSPH i~panelem konfiguracji parametrów fizycznych. Źródło: Własne opracowanie na podstawie dokumentacji biblioteki \cite{SPlisHSPlasH_Library}}
...
która przez ponad dwie dekady rozrastało się o~kolejne warstwy abstrakcji
i~niejawne mechanizmy zarządzania zasobami. Sterownik OpenGL pełnił rolę rozbudowanego 
pośrednika: sam synchronizował polecenia GPU, podejmował decyzje o~alokacji pamięci
i~kompilował shadery w~nieokreślonych momentach, co prowadziło do znacznego narzutu procesora głównego, 
...
\texttt{VkInstance} jest punktem wejścia do środowiska uruchomieniowego Vulkana
i~korzeniem całej hierarchii obiektów API. Jej utworzenie ładuje sterownik Vulkana
...
\begin{tikzpicture}[
  ...
  \draw[arr]   (dev)   -- (pipe);
\end{tikzpicture}
\caption{Hierarchia inicjalizacyjna Vulkana. Źródło: Opracowanie własne na podstawie specyfikacji \cite{vulkan-spec}}
...
\begin{tikzpicture}[
  ...
  \draw[arr] (rc) -- (rm);
\end{tikzpicture}
\caption{Porównanie ścieżek renderowania w~Vulkanie. Źródło: Opracowanie własne na podstawie specyfikacji \cite{vulkan-spec}}
...
\addlinespace
Pośredniczący (Staging) & Tymczasowy bufor w~pamięci widocznej dla procesora, służący do transferu danych inicjalizacyjnych z~CPU do pamięci lokalnej dla urządzenia \\
\bottomrule
\end{tabular}
\end{table}
...
\end{tikzpicture}
\caption{Schemat zarządzania zasobami i~powiązań deskryptorowych w~Vulkanie. Źródło: Opracowanie własne na podstawie specyfikacji \cite{vulkan-spec}}
...
przez shadery SPIR-V. Źródło: Opracowanie własne na podstawie specyfikacji \cite{vulkan-spec}}
...
konkretnego wierzchołka czy cząstki. Do tego celu służy osobne narzędzie opisane
w~kolejnej podsekcji.

\subsection{RenderDoc}

Artefakty wizualne, takie jak błędne pozycje cząstek, nieprawidłowe cieniowanie
...
gdzie $q = r/h$, $r = \|\mathbf{r}\|$ jest odległością między cząstkami,
...
co zapewnia gładkie zanikanie wpływu odległych cząstek.
...
\begin{figure}[ht]
    ...
    funkcji sklejanej w~funkcji znormalizowanej odległości $q = r/h$. Źródło: Opracowanie własne na podstawie \cite{koschier2020smoothed}}
...
\begin{figure}[ht]
    ...
    Wendlanda C2 w~funkcji znormalizowanej odległości $q = r/h$. Źródło: Opracowanie własne na podstawie \cite{wendland1995piecewise}}
...
w~punkcie $q = 0$, co oznacza, że gdy dwie cząstki znajdą się bardzo blisko siebie,
siła odpychająca wynikająca z~jądra zanika. Prowadzi to do efektu skupiania się cząstek
(ang.~\textit{particle clumping}): cząstki, które raz do siebie nadmiernie się zbliżą,
...
jądra Wendlanda należą do grupy wymagającej największej liczby cząstek sąsiadujących,
...
akumulacji błędów gęstości już u~ich źródła. Cząstki nie mają możliwości nabycia
...
\section{Wyszukiwanie sąsiedztwa cząstek}
\label{sec:neighbor_search_theory}

Każda cząstka w~metodzie SPH oddziałuje wyłącznie z~cząstkami znajdującymi się
w~odległości nie większej niż promień wygładzania $h$. Poza tym promieniem wartość
jądra jest równa zero i~dana para cząstek nie wnosi żadnego wkładu do sum
...
Naiwne podejście, polegające na sprawdzeniu każdej pary cząstek, ma złożoność
$\mathcal{O}(N^2)$, gdzie $N$ jest liczbą cząstek. Dla symulacji zawierającej
$10^5$ cząstek oznacza to $10^{10}$ porównań na krok, co jest w~praktyce
nieakceptowalne. Rozwiązaniem jest zastosowanie przestrzennej struktury danych,
które ograniczają proces wyszukiwania do najbliższego otoczenia cząstki, 
...
promieniowi wygładzania $h$. Wówczas sąsiedzi danej cząstki mogą znajdować
...
o~boku równym $h$. Każda cząstka o~pozycji $\mathbf{r}_i$ przypisywana jest
...
cząstek posortowanej tablicy cząstek. Budowa struktury sprowadza się do
posortowania cząstek według ich indeksu komórki, co na GPU realizowane jest
przez szybkie sortowanie pozycyjne \cite{green2010particle}.

Wyszukiwanie sąsiadów dla cząstki $i$ przebiega przez iterację po wszystkich
...
\end{tikzpicture}
\caption{Schemat wyszukiwania sąsiedztwa metodą siatki haszującej w~2D. Źródło: Opracowanie własne na podstawie \cite{green2010particle}}
\label{fig:hashgrid}
\end{figure}

Metoda zapewnia złożoność budowy $\mathcal{O}(N)$ i~złożoność zapytania
$\mathcal{O}(1)$ niezależną od liczby cząstek. Zakłada ona jednak z~góry
...
W~odróżnieniu od podejścia Greena, siatka nie jest jawnie reprezentowana
w~pamięci: przechowywana jest jedynie zwarta lista niepustych komórek, co eliminuje
...
Kluczowym elementem metody jest posortowanie cząstek według indeksu komórki
...
cząstki trafiały do sąsiednich pozycji w~tablicy. Poprawia to lokalność danych
...
\begin{tikzpicture}[
  ...
  \node[lbl, anchor=east] at (-0.4, 1.5) {cząstki};
  ...
  \node[lbl, font=\scriptsize\itshape] at (3.4, -0.65)
    {format: indeks\_komórki : pierwszy\_indeks\_cząstki};
\end{tikzpicture}
\caption{Schemat struktury CLL z~kompaktową tablicą komórek. Źródło: Opracowanie własne na podstawie \cite{band2019compressed}}
\label{fig:cll}
\end{figure}

Drugą cechą wyróżniającą metodę jest kompresja list sąsiedztwa. Ponieważ
cząstki są posortowane, indeksy sąsiadów są niemalejące, a~ich różnice
...
Pozostałe zjawiska fizyczne, mianowicie opór lepki, kohezja cząstek na
...
w~której prędkość cząstki jest nieznacznie uśredniana względem prędkości jej
sąsiadów. Nie modeluje ona jawnie dyssypacji energii, lecz wygładza pole prędkości,
zapobiegając przenikaniu i~lokalnym skupieniom cząstek. Mechanizm ten działa
...
działających na cząstki cieczy leżące przy granicy fazy swobodnej. Cząstka
w~głębi cieczy jest otoczona sąsiadami ze wszystkich stron i~suma sił kohezji
znosi się; cząstka na powierzchni ma sąsiadów tylko po stronie cieczy, co
...
przyspieszenie cząstek leżących przy powierzchni. Bez napięcia powierzchniowego
symulacja pozostaje stabilna, lecz traci zdolność do formowania kropli
i~realistycznego zachowania cienkiej warstwy cieczy. W~niniejszej pracy napięcie
...
Granice domeny symulacji wprowadzają fundamentalny problem geometryczny: cząstka
...
cząstek. Sytuację tę ilustruje rysunek~\ref{fig:boundary_support}.
...
\begin{tikzpicture}[scale=1.3,
  ...
  \node[font=\scriptsize\sffamily, black!50] at (-0.1, 0.4)
    {\textit{duch}};
\end{tikzpicture}
\caption{Problem niepełnego sąsiedztwa przy granicy domeny. Źródło: Opracowanie własne na podstawie \cite{schechter2012ghost}}
...
ciśnienia generuje w~pobliżu ścian fałszywy gradient przyciągający cząstki do
granicy. Gradient ciśnienia jest dodatkowo błędnie skierowany, ponieważ suma
po sąsiadach nie jest symetrycznie rozkładana wokół cząstki. Przy dużych
krokach czasowych możliwe jest też przenikanie cząstek przez ścianę.

Powszechnym rozwiązaniem w~literaturze są cząstki-duchy \cite{schechter2012ghost},
tworzone symetrycznie po drugiej stronie granicy jako odbicia cząstek fluidu.
...
od ściany. Alternatywą są stałe warstwy cząstek brzegowych reprezentujące
geometrię przeszkody, których cząstki uczestniczą w~sumach SPH, lecz nie podlegają
integracji ruchu. W~niniejszej pracy zastosowano uproszczone podejście: po każdym
kroku całkowania pozycja cząstek przekraczających granicę domeny jest korygowana
...
Wynikiem każdego kroku symulacji DFSPH jest zbiór $N$ cząstek z~zaktualizowanymi
pozycjami. Cząstki stanowią dyskretne punkty masy; same w~sobie nie tworzą
...
izopowierzchni metodą przemarszu promienia oraz obliczenie koloru piksela z~uwzględnieniem
...
\begin{tikzpicture}[
  ...
  \node[box] (sim) at (0,    0)   {Cząstki\\SPH};
  \node[box] (spl) at (3.6,  0)   {Splatting\\(siatka wokseli)};
  \node[box] (tex) at (7.2,  0)   {3D tekstura\\gęstości};
  \node[box] (ray) at (10.8, 0)   {Przemarsz promienia\\(izopowierzchnia)};
  ...
  \draw[arr] (shd) -- (px);
\end{tikzpicture}
\caption{Potok wizualizacji: od listy cząstek SPH do końcowego koloru piksela. Źródło: Opracowanie własne}
\label{fig:vis_pipeline}
\end{figure}

\subsection{Rekonstrukcja pola gęstości}

Pierwszym krokiem potoku jest przekształcenie listy pozycji cząstek w~trójwymiarową
...
cząstki $i$ wyznaczany jest odpowiadający jej woksel centralny $\mathbf{c}_i$,
...
Ponieważ każda cząstka przetwarzana jest przez osobny wątek GPU, wiele wątków
...
konieczności synchronizacji. Wynikiem etapu rzutowania cząstek na siatkę wokseli (ang.~\textit{splatting}) jest trójwymiarowa tekstura
...
\subsection{Wyznaczanie izopowierzchni metodą Raymarching}
...
\end{equation}
Wyznaczenie przecięcia promienia kamery z~tą powierzchnią realizuje przemarsz promienia \cite{quilez2008raymarching, tomczak2012gpu}: promień emitowany ze
...
Przed rozpoczęciem przemarszu promienia wykonywany jest test przecięcia promienia
...
fragmentów, zanim pętla przemarszu w~ogóle się rozpocznie.
...
\end{tikzpicture}
\caption{Schemat przemarszu promienia z~binarnym zawężaniem. Białe kółka to próbki poniżej
         progu gęstości, szare kółko to pierwsza próbka powyżej progu
         (ciecz). Bisekcja przedziału $[t_4,\, t_5]$ wyznacza punkt powierzchni
         $\mathbf{p}_s$ z~dokładnością $\Delta t / 2^8$. Źródło: Opracowanie własne na podstawie \cite{quilez2013advancing}}
...
Kierunek odbitego promienia próbkuje mapę środowiskową, kierunek załamanego wyznaczany jest
