...
\label{sec:app_presentation}

Aplikacja stanowi interaktywny silnik symulacji cieczy działający w~czasie rzeczywistym,
łączący metodę DFSPH z~wolumetrycznym renderowaniem na GPU.
Po uruchomieniu użytkownik widzi scenę 3D z~prostopadłościennym obszarem symulacji,
wewnątrz którego odbywa się symulacja cieczy, oraz panel konfiguracyjny
umożliwiający modyfikację parametrów w~czasie rzeczywistym
(rysunek~\ref{fig:app_window}).

Motywacją do stworzenia projektu była chęć głębszego zrozumienia sposobu, w~jaki
współczesne silniki gier i~aplikacji realizują symulacje cząsteczkowe --- symulacja
cieczy metodą SPH stanowi pod tym względem szczególnie reprezentatywny przypadek,
ponieważ łączy w~sobie wszystkie kluczowe wyzwania obliczeń równoległych:
nieregularny dostęp do danych sąsiedztwa, iteracyjne solvery zbieżnościowe
i~konieczność ścisłej synchronizacji między setkami tysięcy wątków GPU.
Realizacja projektu pozwoliła na praktyczne opanowanie programowania równoległego
w~modelu GPGPU oraz pracy z~nowoczesnym interfejsem graficznym Vulkan,
który wymaga jawnego zarządzania synchronizacją, pamięcią i~potokami poleceń ---
w~odróżnieniu od bardziej abstrakcyjnych API jak OpenGL czy DirectX 11.

Silnik może służyć jako platforma badawcza do testowania algorytmów symulacji cieczy,
demonstracyjna aplikacja edukacyjna lub punkt wyjścia do integracji fizyki cieczy
z~systemem renderowania w~czasie rzeczywistym.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/app_window.png}
\caption{Okno aplikacji podczas symulacji. Źródło: Opracowanie własne}
...
\midrule
\texttt{Engine}            & CPU          & Pętla zdarzeń \texttt{winit}, timing, zarządzanie sceną i~wejściem \\[3pt]
...
\texttt{Pipelines}         & GPU          & Potoki graficzne: niebo, cząstki, woda (przemarsz promienia), granice \\
\bottomrule
\end{tabular}
\end{table}

Pętla ramki jest sterowana zdarzeniem \texttt{RedrawRequested} biblioteki
\texttt{winit} i~wykonuje się w~trybie \texttt{ControlFlow::Poll}, co oznacza brak
...
\If{\upshape tryb przemarszu promienia}{
...
Architektura silnika opiera się na kilku abstrakcjach rozdzielających odpowiedzialności między komponentami.
\textit{ApplicationHandler} --- cecha biblioteki \texttt{winit} --- realizuje
odwrócenie kontroli nad pętlą zdarzeń, omówione szczegółowo
w~podrozdziale~\ref{subsec:window}.
\textit{ComputeStep} jest autorską abstrakcją nadającą wszystkim potokom
obliczeniowym GPU jednolity interfejs, rozdzielającą jednorazową inicjalizację
zasobów od wielokrotnie powtarzanego zapisu poleceń do bufora komend.
Do obsługi wejścia zastosowano wzorzec projektowy \textit{Command}~\cite{gamma1994design}:
każde naciśnięcie klawisza lub ruch myszy przekładane są na obiekt komendy,
który wie, jak zmienić scenę, ale nie wie, skąd pochodzi sygnał.
\textit{Actor} to uproszczona koncepcja popularna w~społeczności twórców gier
--- stosowana m.in.\ w~Unreal Engine~5~\cite{ue5fluids} ---
abstrahująca przemieszczanie i~obrót obiektu w~przestrzeni 3D niezależnie
od jego konkretnego typu.
...
biblioteka \texttt{winit} zarządzająca oknem i~pętlą zdarzeń, model kamery
...
Okno i~pętla zdarzeń są obsługiwane przez bibliotekę \texttt{winit} za pośrednictwem
...
przed tym zdarzeniem jest niemożliwe. \texttt{Winit} zaleca\cite{winit} więc inicjalizację okna
...
oczekiwania na zdarzenia systemowe. Łańcuch wymiany używa trybu \texttt{PresentMode::Immediate}, który wyświetla
ukończoną klatkę natychmiast, bez oczekiwania na sygnał synchronizacji
pionowej. Liczba obrazów łańcucha wymiany wynosi 3; stała
\texttt{MAX\_FRAMES\_IN\_FLIGHT = 3} ogranicza liczbę ramek budowanych
przez CPU równolegle z~pracą GPU --- razem tworzą pełne potrójne buforowanie.
...
\texttt{inv\_view\_proj}& \texttt{Mat4} & Odwrotność $(\mathbf{P}\mathbf{V})^{-1}$ --- używana przez przemarsz promienia do odtworzenia kierunku promienia \\[3pt]
...
\begin{tikzpicture}[
  ...
  \draw[->] (inv)   -- (buf);
\end{tikzpicture}
\caption{Łańcuch obliczeń kamery. Źródło: Opracowanie własne}
\label{fig:camera_chain}
\end{figure}

Macierz \texttt{inv\_view\_proj} jest potrzebna dla przemarszu promienia. Shader
...
rysunek~\ref{fig:egui_params} przedstawia wygląd panelu konfiguracyjnego
w~interfejsie \texttt{egui} podczas działania aplikacji.

\begin{figure}[htbp]
\centering
\includegraphics[angle=90, width=0.75\linewidth]{figures/egui_params.png}
\caption{Panel parametrów symulacji w~interfejsie \texttt{egui}. Źródło: Opracowanie własne}
\label{fig:egui_params}
\end{figure}
...
Konfiguracja silnika Vulkan skupia się wokół dwóch elementów:
...
deklarowania kompatybilności potoków z~konkretnym formatem bufora ramki.
...
Metoda \texttt{acquire} pobiera indeks dostępnego obrazu z~łańcucha wymiany
...
obliczeniowy odpowiadający kolejnym etapom pętli ramki
z~algorytmu~\ref{alg:frame_loop}.
...
\texttt{DensitySourceTermPipeline}     & Wyraz źródłowy korekcji gęstości \\[3pt]
...
\texttt{DivergenceSourceTermPipeline}  & Wyraz źródłowy korekcji dywergencji \\[3pt]
...
Zanim zbudowany zostanie potok obliczeniowy, konieczne jest podjęcie
...
Rozwiązaniem jest podwójne buforowanie,
...
pierwsze stałe redukujące kolizje przy typowych rozkładach cząstek~\cite{teschner2003optimized},
...
\end{tikzpicture}
\caption{Czterostopniowy potok wyszukiwania sąsiadów. Źródło: Opracowanie własne}
...
potoku wyszukiwania sąsiadów i~wywiera bezpośredni wpływ na czas wykonania klatki.
Pierwszą zaimplementowaną metodą było sortowanie bitoniczne --- algorytm oparty na
sieci porównań, który dobrze nadaje się na GPU ze względu na zerową
rozbieżność wątków (ang.~\textit{thread divergence}) i~prostą strukturę sterowania.
...
sortowanie.
Skłoniło to do implementacji sortowania pozycyjnego --- algorytmu liniowego
...
\subsection{Sortowanie bitoniczne}
...
rozbieżność wątków --- jedno z~podstawowych źródeł degradacji
...
Sortowanie pozycyjne jest algorytmem sortowania przez zliczanie, a~nie przez porównywanie, co stanowi
...
\begin{tikzpicture}[
  ...
  \draw[arr] (ror)  -- (out);
\end{tikzpicture}
\caption{Przepływ danych w~jednym przebiegu sortowania pozycyjnego. Źródło: Opracowanie własne na podstawie~\cite{harada2011introduction}}
...
\texttt{ELEMENTS\_PER\_INVOCATION}). Każdy wątek wyodrębnia $b$-bitową cyfrę
z~każdego ze swoich ośmiu elementów przez przesunięcie bitowe i~maskowanie, a~następnie
...
$d \cdot m + \mathrm{wg}$, gdzie $d$ to wartość cyfry, a~$\mathrm{wg}$ --- numer
...
go pod ten adres w~buforze wyjściowym. Operacja ta jest stabilnym rozproszonym zapisem (ang.~\textit{scatter}) ---
zachowuje wzajemną kolejność elementów o~tej samej wartości cyfry, co jest
...
Parzyste przebiegi kierują dane z~\texttt{grid\_entries} do \texttt{entries\_tmp},
nieparzyste --- w~odwrotnym kierunku; po czterech przebiegach wynik jest zawsze
z~powrotem w~\texttt{grid\_entries}. Złożoność $O(kN)$ ze stałym $k=4$
sprawia, że koszt sortowania rośnie liniowo z~liczbą cząstek, co czyni sortowanie pozycyjne
...
\begin{table}[htbp]
\centering
\caption{Porównanie własności sortowania bitonicznego i~pozycyjnego}
\label{tab:sort_comparison}
\begin{tabular}{p{4.5cm}p{4.5cm}p{4.5cm}}
\toprule
\textbf{Cecha} & \textbf{Sortowanie bitoniczne} & \textbf{Sortowanie pozycyjne} \\
\midrule
Złożoność czasowa & $O(N \log^2 N)$ & $O(kN)$, $k=4$ \\[3pt]
Liczba dispatchów dla $N=2^{15}$ & 120 & 12 \\[3pt]
Rozbieżność wątków & Zerowa & Zerowa \\[3pt]
Pamięć pomocnicza & Brak (sort. w~miejscu) & Bufor \texttt{entries\_tmp} i~bufor liczników \\[3pt]
...
elementów $N$, w~skali logarytmicznej na obu osiach. Źródło: Opracowanie własne}
...
gdy każdy przebieg sortowania pozycyjnego wymaga trzech kolejnych dispatchów ze wzajemną
...
stałą liczbą dwunastu wywołań sortowania pozycyjnego. Dla $N = 2^{22}$ (ponad cztery miliony
elementów) sortowanie pozycyjne jest niemal trzykrotnie szybsze: $30{,}5$~ms
...
(tabele~\ref{tab:tracy_bitonic} i~\ref{tab:tracy_radix}) pokazuje, że średni
...
dla wariantu sortowania pozycyjnego. Oba pomiary dotyczą tej samej konfiguracji symulacji,
więc niemal sześciokrotna różnica wynika wyłącznie z~wyboru algorytmu
sortowania.

\begin{table}[htbp]
\centering
\caption{Profil wydajności dla wariantu z~sortowaniem bitonicznym ($N=25\,000$ cząstek)}
\label{tab:tracy_bitonic}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llrrr}
\toprule
\textbf{Nazwa etapu} & \textbf{Czas całkowity} & \textbf{Wywołania} & \textbf{Średni czas} \\
\midrule
\texttt{neighbor\_search\_post\_integrate}  & 37,40 s (14,24\%)  & 10\,276 & 3,64 ms \\
\texttt{neighbor\_search\_init}             & 5,69 s (2,17\%)    & 1\,468  & 3,88 ms \\
\texttt{divergence\_solver}                 & 3,48 s (1,32\%)    & 10\,276 & 338,46 $\mu$s \\
\texttt{density\_solver}                    & 3,43 s (1,31\%)    & 10\,276 & 333,98 $\mu$s \\
\texttt{density\_alpha\_post\_integrate}    & 477,31 ms (0,18\%) & 10\,276 & 46,45 $\mu$s \\
\texttt{divergence\_source\_term}           & 425,08 ms (0,16\%) & 10\,276 & 41,37 $\mu$s \\
\texttt{density\_source\_term}              & 421,67 ms (0,16\%) & 10\,276 & 41,03 $\mu$s \\
\texttt{viscosity}                          & 412,09 ms (0,16\%) & 10\,276 & 40,10 $\mu$s \\
\texttt{pressure\_integration}              & 391,08 ms (0,15\%) & 10\,276 & 38,06 $\mu$s \\
\texttt{divergence\_integration}            & 294,14 ms (0,11\%) & 10\,276 & 28,62 $\mu$s \\
\texttt{density\_alpha\_init}               & 63,68 ms (0,02\%)  & 1\,468  & 43,38 $\mu$s \\
\texttt{stats}                              & 58,12 ms (0,02\%)  & 1\,468  & 39,59 $\mu$s \\
\texttt{substep}                            & 12,80 ms (0,00\%)  & 10\,276 & 1,25 $\mu$s \\
\bottomrule
\end{tabular}%
}
\vspace{1ex} \\
...
\begin{table}[htbp]
\centering
\caption{Profil wydajności dla wariantu z~sortowaniem pozycyjnym ($N=25\,000$ cząstek)}
\label{tab:tracy_radix}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrr}
\toprule
\textbf{Nazwa etapu} & \textbf{Czas całkowity} & \textbf{Wywołania} & \textbf{Średni czas} \\
\midrule
\texttt{neighbor\_search\_post\_integrate}  & 1,21 s (4,95\%)    & 1\,960 & 616,30 $\mu$s \\
\texttt{divergence\_solver}                 & 634,15 ms (2,60\%) & 1\,960 & 323,54 $\mu$s \\
\texttt{density\_solver}                    & 624,33 ms (2,56\%) & 1\,960 & 318,54 $\mu$s \\
\texttt{neighbor\_search\_init}             & 78,88 ms (0,32\%)  & 98     & 804,93 $\mu$s \\
\texttt{density\_source\_term}              & 70,83 ms (0,29\%)  & 1\,960 & 36,14 $\mu$s \\
\texttt{density\_alpha\_post\_integrate}    & 70,20 ms (0,29\%)  & 1\,960 & 35,82 $\mu$s \\
\texttt{divergence\_source\_term}           & 70,08 ms (0,29\%)  & 1\,960 & 35,75 $\mu$s \\
\texttt{pressure\_integration}              & 68,99 ms (0,28\%)  & 1\,960 & 35,20 $\mu$s \\
\texttt{viscosity}                          & 67,89 ms (0,28\%)  & 1\,960 & 34,64 $\mu$s \\
\texttt{divergence\_integration}            & 52,44 ms (0,21\%)  & 1\,960 & 26,76 $\mu$s \\
\texttt{stats}                              & 5,16 ms (0,02\%)   & 98     & 52,65 $\mu$s \\
\texttt{density\_alpha\_init}               & 3,78 ms (0,02\%)   & 98     & 38,54 $\mu$s \\
\texttt{substep}                            & 1,01 ms (0,00\%)   & 1\,960 & 514 ns \\
\bottomrule
\end{tabular}%
}
\vspace{1ex} \\
...
Rozbieżność między izolowanym testem porównawczym (gdzie sortowanie pozycyjne dla $N = 2^{15}$ jest
nieznacznie wolniejsze) a~pomiarem produkcyjnym (gdzie ten sam $N$ daje
...
\texttt{AutoCommandBufferBuilder}; sortowanie pozycyjne wymaga ich jedynie dwunastu. W~teście
...
od kilku tysięcy cząstek wzwyż sortowanie pozycyjne jest szybszym wariantem, a~jego
...
Ta różnica determinuje kolejność shaderów w~pętli podkroków (ang. substep).
...
pełną pętlę podkroków ustalającą kolejność wszystkich etapów wewnątrz
...
\subsection{Pętla podkroków}
\label{subsec:substep_loop}

Pełna sekwencja wywołań shaderów w~obrębie podkroku ukazana jest
...
podkroku nie jest redundancją --- jest koniecznością wynikającą z~fizycznego
przemieszczenia cząstek.
Tablica \texttt{grid\_start}, zbudowana przed podkrokiem, indeksuje pozycje
...
Iteracyjny charakter obu solverów wynika z~natury problemu.
...
Shader \texttt{density\_and\_alpha} oblicza w~jednym przebiegu dwie
...
\begin{lstlisting}[caption={Shader \texttt{density\_and\_alpha} --- logika akumulacji},
...
Każda wywołanie odpowiada jednej cząstce~$i$.
...
Shader \texttt{density\_source\_term} oblicza dla każdej cząstki~$i$
wyraz źródłowy łączący chwilowy deficyt gęstości z~prognozą jego narastania
na podstawie prędkości adwekcyjnych wyznaczonych po fazie lepkości:

\begin{lstlisting}[caption={Shader \texttt{density\_source\_term} --- wyraz źródłowy solvera gęstości},
...
Pierwszy shader wyznacza dla każdej cząstki przyspieszenie ciśnieniowe
będące iloczynem macierzy układu i~wektora ciśnień; drugi aktualizuje ciśnienie:

\begin{lstlisting}[caption={Shader \texttt{pressure\_update} --- krok Jacobiego},
...
wyraźną dwumodalność rozkładu gęstości (rysunek~\ref{fig:rest_density}): 
...
Właśnie ten deficyt powoduje plateau widoczne w~teście zbieżności (rysunek~\ref{fig:convergence}a) --- błąd $\overline{|\rho_i - \rho_0|}$ 
...
\begin{figure}[htbp]
    ...
    \caption{Histogram gęstości w~konfiguracji spoczynkowej ($n = 4800$ cząstek, bez kroku symulacji). Źródło: opracowanie własne}
...
Shader \texttt{divergence\_source\_term} oblicza wyraz źródłowy z~prędkości bezpośrednio po całkowaniu ciśnień:

\begin{lstlisting}[caption={Shader \texttt{divergence\_source\_term} --- wyraz źródłowy solvera dywergencji},
...
W~odróżnieniu od solvera gęstości, wyraz źródłowy nie zawiera składnika $(\rho_0 - \rho_i)/\Delta t$ --- celem jest wyłącznie zerowanie dywergencji.
Pętla Jacobiego jest identyczna (\texttt{pressure\_force} i~\texttt{pressure\_update}), natomiast shader \texttt{divergence\_integration} stosuje wynikowe ciśnienia jako korekty prędkości, nie pozycji:

\begin{lstlisting}[caption={Shader \texttt{divergence\_integration} --- całkowanie prędkości},
                   label=lst:divergence_integration,]
vec3 new_vel = vel + ap * dt;
velocities[i] = vec4(new_vel, 0.0);
\end{lstlisting}

Rysunek~\ref{fig:convergence}b ujawnia odwrotną zależność:
...
\begin{figure}[htbp]
    ...
    \caption{Zbieżność solvera DFSPH: uśredniony błąd gęstości~(a) i~błąd dywergencji~(b)
             w~funkcji liczby iteracji Jacobiego. Źródło: Opracowanie własne}
    \label{fig:convergence}
\end{figure}

\subsection{Warunek CFL i~dobór \texorpdfstring{$\Delta t$}{Delta t}}
\label{subsec:cfl}
...
Rysunek~\ref{fig:cfl_comparison} zestawia obie strategie: panel~(a) pokazuje krok
czasowy --- wariant adaptacyjny rośnie do $50~\mathrm{ms}$, po czym w~okolicach
klatki~205 wywołuje katastrofalną niestabilność ($v_{\max} \approx 1100~\mathrm{m/s}$);
panel~(b) pokazuje liczbę iteracji solvera --- wariant adaptacyjny osiąga limit
100 iteracji w~każdej klatce bez wyjątku; panel~(c) pokazuje $v_{\max}$ ---
...
\begin{figure}[htbp]
  ...
  \caption{Porównanie strategii statycznego i~adaptacyjnego $\Delta t$
    w~trakcie 500 klatek symulacji: krok czasowy~(a),
    liczba iteracji solvera Jacobiego~(b), maksymalna prędkość cząstki~(c). Źródło: Opracowanie własne}
...
na cząstki poza solverem DFSPH: wygładzenia prędkości metodą XSPH oraz
odpowiedzi na kolizje ze ścianami sześcianu symulacji.
...
dopuszczają widoczne przenikanie cząstek przy zderzeniach,
...
Grawitacja dodawana jest w~tym samym przebiegu tuż po korekcji XSPH,
...
Stała \texttt{eps} przesuwa cząstkę o~$1~\mathrm{mm}$ ponad granicę,
...
ruch równoległy do ściany, nie zatrzymując cząstki całkowicie.
...
zapobiega ucieczce cząstek poza granice sześcianu symulacji i~jest wystarczające dla
...
Potok wizualizacji przekształca listę pozycji cząstek w~gotowy kolor piksela
w~trzech etapach: splatting gęstości do trójwymiarowej tekstury, przemarsz promienia
...
\subsection{Splatting gęstości do tekstury 3D}
\label{subsec:density_splat}

Shader \texttt{splat\_density} dla każdej cząstki wyznacza woksel centralny,
...
przedział $[t_{\min}, t_{\max}]$ wejścia i~wyjścia promienia z~obszaru symulacji;
jeśli $t_{\min} > t_{\max}$, promień w~ogóle nie przecina sześcianu otaczającego i~jest odrzucany instrukcją \texttt{discard}.
...
\texttt{STEP\_SIZE} działa poprawnie niezależnie od wymiarów sześcianu otaczającego.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{figures/debug_normals.png}
\caption{Wizualizacja normalnych powierzchni cieczy zakodowanych jako kolor RGB. Źródło: Opracowanie własne}
\label{fig:debug_normals}
\end{figure}

Rysunek~\ref{fig:debug_normals} ilustruje wyznaczone normalne zakodowane jako kolor RGB.
...
pionowym w~miejscach, gdzie gradient zanika.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\linewidth]{figures/debug_thickness.png}
\caption{Wizualizacja grubości cieczy wzdłuż promienia jako skala szarości.
         Źródło: Opracowanie własne}
\label{fig:debug_thickness}
\end{figure}

Na rysunku~\ref{fig:debug_thickness} przedstawiono grubość cieczy jako skalę szarości ---
ciemniejsze obszary odpowiadają cienkim warstwom przy brzegach, jaśniejsze głębokim
kolumnom wewnątrz bryły, co bezpośrednio steruje absorpcją Beer-Lamberta.
...
odbitego i~załamanego kierunku próbkują mapę środowiskową:
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\linewidth]{figures/debug_reflection.png}
\caption{Wizualizacja składnika odbicia zwierciadlanego powierzchni cieczy.
         Źródło: Opracowanie własne}
\label{fig:debug_reflection}
\end{figure}

Rysunek~\ref{fig:debug_reflection} pokazuje składnik odbicia bez refrakcji.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/debug_refraction.png}
\caption{Wizualizacja składnika refrakcji z~absorpcją i~rozproszeniem podpowierzchniowym (SSS).
         Źródło: Opracowanie własne}
\label{fig:debug_refraction_sss}
\end{figure}

Rysunek~\ref{fig:debug_refraction_sss} prezentuje składnik refrakcji z~absorpcją i~SSS.
Grubość \texttt{thickness} mierzona jest osobnym przelotem 64 próbek od punktu
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{figures/skybox_hdri.png}
\caption{Panorama środowiskowa HDRI użyta jako tło i~źródło oświetlenia sceny.
         Źródło: Opracowanie własne}
\label{fig:skybox_hdri}
\end{figure}

Użyta panorama HDRI przedstawiona jest na rysunku~\ref{fig:skybox_hdri}.
...
Mapa środowiskowa renderowana jest osobnym shaderem \texttt{sky.frag} na sferze
