...
Типи (type), які тобі знадобляться:feat: Нова функціональність (наприклад, додав SPH).fix: Виправлення помилки.refactor: Зміна коду без зміни функціоналу (твій поточний випадок!).docs: Документація (README, коментарі).chore: Технічні завдання (зміна .gitignore, оновлення залежностей у Cargo.toml).perf: Покращення продуктивності.Область (scope):Це те, на що вплинув коміт: renderer, engine, physics, constants.2. Приклад логічного формату для твого проектуПриклад комітуКоли використовуватиfeat(renderer): implement raymarching water surfaceДодав нову велику фічу.refactor(engine): migrate to VulkanoWindowRendererПереписав архітектуру на нову бібліотеку.fix(physics): resolve density pressure instabilityВиправив баг у розрахунках SPH.chore(deps): update vulkano to version 0.34
---
